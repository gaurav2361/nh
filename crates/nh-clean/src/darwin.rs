...
pub fn run_clean_darwin(
  args: &DarwinCleanArgs,
  _elevate: ElevationStrategy,
) -> Result<()> {
    ...
    if args.dry {
      println!(
        ...
        Paint::new(format!(
          ...
          bytes_to_human(total_freed + total_potential)
        ))
        .bold()
      );
    } else {
      println!(
        ...
        Paint::new(format!(
          "Cleanup complete! Total space freed: {}",
          bytes_to_human(total_freed)
        ))
        .bold()
      );
    }
...
fn clean_user(args: &DarwinCleanArgs) -> Result<u64> {
  ...
  freed +=
    safe_remove_contents(&format!("{}/Library/Caches", home), args, false)?;

  // User Logs (Clean contents)
  freed +=
    safe_remove_contents(&format!("{}/Library/Logs", home), args, false)?;

  // Trash (Clean contents)
  // 1. Primary User Trash
  let trash_path = format!("{}/.Trash", home);
  let trash_size = get_size(Path::new(&trash_path)).unwrap_or(0);
  ...
  if args.dry {
    if trash_size > 0 {
      println!(
        ...
        Paint::new(ICON_DRY_RUN).fg(YELLOW).bold(),
        trash_path,
        bytes_to_human(trash_size)
      );
      freed += trash_size;
    }
  } else {
    let mut emptied = false;
    // Try AppleScript first as it's more reliable for system trash
    if !args.ask {
      ...
      if let Ok(s) = status {
        if s.success() {
          println!(
            "  {} Emptied Trash via Finder{}",
            Paint::new(ICON_SUCCESS).fg(GREEN).bold(),
            if trash_size > 0 {
              format!(" ({})", bytes_to_human(trash_size))
            } else {
              "".to_string()
            }
          );
    ...
    if !emptied {
      // If AppleScript failed or we're in 'ask' mode, try manual removal
      // We only do this if the directory is readable or we know it has content
      if trash_size > 0 || Path::new(&trash_path).exists() {
        freed += safe_remove_contents(&trash_path, args, false)?;
      }
    }
  }

  // 2. iCloud Drive Trash
  freed += safe_remove_contents(
    &format!(
  ...
  // 3. Other volumes' Trashes for current user
  let uid = unistd::getuid().as_raw();
  if let Ok(volumes) = std::fs::read_dir("/Volumes") {
    for volume in volumes.flatten() {
      let trash_path = volume.path().join(".Trashes").join(uid.to_string());
      if trash_path.exists() {
        freed +=
          safe_remove_contents(&trash_path.to_string_lossy(), args, true)?;
      }
    }
  }
...
fn clean_dev(args: &DarwinCleanArgs) -> Result<u64> {
  println!(
    "\n{} {}",
    Paint::new(ICON_ARROW).fg(PURPLE).bold(),
    Paint::new("Developer Tools Cleanup").bold()
  );
  let mut freed = 0;
  let home = std::env::var("HOME")?;
  ...
  // Rust/Cargo
  freed +=
    safe_remove(&format!("{}/.cargo/registry/cache", home), args, false)?;
  freed += safe_remove(&format!("{}/.cargo/git/db", home), args, false)?;

  // Go
  freed +=
    safe_remove(&format!("{}/Library/Caches/go-build", home), args, false)?;
...
fn clean_containers(args: &DarwinCleanArgs) -> Result<u64> {
    ...
    if args.dry {
      ...
      if status.is_ok() && status.unwrap().success() {
        ...
        println!(
          ...
          Paint::new("SUCCESS:").fg(Color::Green)
        );
      }
    }
  }
  ...
  freed +=
    safe_remove(&format!("{}/.colima/default/cache", home), args, false)?;
...
fn clean_apps(args: &DarwinCleanArgs) -> Result<u64> {
  println!(
    "\n{} {}",
    Paint::new(ICON_ARROW).fg(PURPLE).bold(),
    Paint::new("Application Cleanup").bold()
  );
  let mut freed = 0;
  let home = std::env::var("HOME")?;
  ...
  // Slack
  freed += safe_remove(
    &format!("{}/Library/Application Support/Slack/Cache", home),
    args,
    false,
  )?;
  ...
  // Discord
  freed += safe_remove(
    &format!("{}/Library/Application Support/discord/Cache", home),
    args,
    false,
  )?;
...
fn clean_browsers(args: &DarwinCleanArgs) -> Result<u64> {
  println!(
    "\n{} {}",
    Paint::new(ICON_ARROW).fg(PURPLE).bold(),
    Paint::new("Browser Cleanup").bold()
  );
  let mut freed = 0;
  let home = std::env::var("HOME")?;
...
fn clean_purge(args: &DarwinCleanArgs) -> Result<u64> {
  println!(
    "\n{} {}",
    Paint::new(ICON_ARROW).fg(PURPLE).bold(),
    Paint::new("Project Purge (Build Artifacts)").bold()
  );
...
fn run_optimize(args: &DarwinCleanArgs) -> Result<()> {
  println!(
    "\n{} {}",
    Paint::new(ICON_ARROW).fg(PURPLE).bold(),
    Paint::new("System Optimization").bold()
  );
  ...
  if args.dry {
    ...
    println!(
      "  {} Would rebuild launch services",
      Paint::new(ICON_DRY_RUN).fg(YELLOW).bold()
    );
    println!(
      "  {} Would refresh Finder and Dock",
      Paint::new(ICON_DRY_RUN).fg(YELLOW).bold()
    );
    return Ok(());
  }
...
fn clean_nix(args: &DarwinCleanArgs) -> Result<()> {
  println!(
    "\n{} {}",
    Paint::new(ICON_ARROW).fg(PURPLE).bold(),
    Paint::new("Nix Cleanup").bold()
  );

  if args.dry {
    println!(
      "  {} Would run nix-collect-garbage -d (Removes all old generations)",
      Paint::new(ICON_DRY_RUN).fg(YELLOW).bold()
    );
    println!(
      "  {} Would run nix-store --gc (Reclaims store space)",
      Paint::new(ICON_DRY_RUN).fg(YELLOW).bold()
    );
    println!(
      "  {} Would run nix-store --optimise (Deduplicates store files)",
      Paint::new(ICON_DRY_RUN).fg(YELLOW).bold()
    );
    return Ok(());
  }
  ...
  match std::fs::read_dir(path) {
    Ok(entries) => {
      for entry in entries.flatten() {
        let entry_path = entry.path();
        total_freed +=
          safe_remove(&entry_path.to_string_lossy(), args, needs_sudo)?;
      }
    },
    Err(e) => {
      warn!(
        "Failed to read directory {}: {}. Access may be restricted (check Full Disk Access).",
        path_str, e
      );
    },
  }
